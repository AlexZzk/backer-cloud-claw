...
export class HttpServer {
  ...
  private async _buildSceneHints(): Promise<{
    ...
    const activeTaskWorkerIds = new Set<string>();
    const now = Date.now();
    ...
    for (const worker of this.config.workers ?? []) {
      const tasks = await this.taskStore.list(worker.id, ['todo', 'in_progress']);
      if (tasks.length > 0) activeTaskWorkerIds.add(worker.id);
      if (tasks.some(t => t.priority === 'urgent' || t.priority === 'high')) {
        focusWorkerIds.add(worker.id);
      }
    }

    return { meetingWorkerIds, focusWorkerIds, activeTaskWorkerIds };
  }
