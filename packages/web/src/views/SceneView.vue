        ...
        <div v-for="zone in scene.zones" :key="zone.id" class="zone-card">
          ...
          <div class="avatars">
            <div
              ...
              :title="`${entity.displayName} · ${entity.activityLabel}${entity.seatId ? ` · ${entity.seatId}` : ''}`"
            >
              {{ entity.displayName }}
              <span v-if="entity.assetBinding" class="asset-badge">#{{ entity.assetBinding.assetId }}</span>
              <span v-if="entity.seatId" class="seat-badge">@{{ entity.seatId }}</span>
            </div>
...
.seat-badge { margin-left: 4px; opacity: 0.8; font-size: 10px; }
