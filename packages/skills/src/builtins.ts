...
**不得将原始人称词原样照抄转发，必须转换为具体的人名或 Worker ID。**

转换原则：
- 用户（主管）说的"我"→ 在转发给其他 Worker 时写成「用户」或「你的主管」
- 原消息中"通知我"→ 转发时写成「通知 [发送方的 Worker ID]」或「完成后在本群回复」
- 原消息中"提醒我"→ 转发时写成「提醒 [具体人名/ID]」

**错误示例**（照搬人称）：
> 用户说"帮我提醒worker2完成报告"
> ❌ 转发给 worker2：「@worker2，请提醒我完成报告」

**正确示例**（明确主语）：
> ✅ 转发给 worker2：「@worker2，请在报告完成后通知 [你的Worker ID]，由你负责跟进并向用户汇报结果」

**链式委托示例**（A → B → C）：
> A 收到用户指令："让B告诉C执行任务，完成后通知我"
> A 发给 B：「@[B的ID]，请让C执行[任务]。完成后请通知 [A的ID]，由 [A的ID] 向用户汇报。」
> B 发给 C：「@[C的ID]，请执行[任务]。完成后请通知 [B的ID]。」（B负责向A汇报，不是直接找用户）`,
    prompt: '',
    source: 'builtin',
  },
