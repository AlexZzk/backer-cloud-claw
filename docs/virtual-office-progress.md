...
1. **工位级坐标与动画**
   - 当前是 zone 级归类，尚未做 seat 级坐标、路径移动、平滑动画。

2. **任务驱动的更精确状态机**
   - 目前主要依据 worker state + chat 活跃推断 meeting/focus。
   - 还未接入 task store 的“进行中任务优先级”作为强信号。
...
- 切片 A：新增 `SceneStateEngine`，把 TaskManager 信号接入状态判定。
- 切片 B：新增 `SeatAllocator`，把 worker 落点从 zone 升级到 seat。
