...
**私有技能晋升为团队技能：**
- 触发方：用户在 UI 发起（看到某 Worker 的私有技能有价值）
- 或 Worker 申请"公开分享"→ 审核员审批 → Admin 同意 → 提升为 Shared 技能
...
**Auditor 不在线 / 人类不在线时的策略：**
- 高风险操作：进入等待队列，任务标记为 `blocked`
- 等待超时（可配置，如 24h）：任务失败并通知 Admin
- 可预配置"快速白名单"：指定路径下的操作不需审批（如 `rm tmp/` 内文件）

**Auditor 是否唯一：**
- 系统默认有且仅有**一个 Auditor**（简化信任链）
- 高级用法：可配置多个 Auditor，但最终 Admin 审批只需一人同意
...
### D1：私有技能晋升为团队技能，审批由谁发起？

- **选项 A**：用户（Admin）在 UI 直接操作
- **选项 B**：Worker 申请 → Auditor 审批 → Admin 确认（三步）
- **当前倾向**：选项 A（简单，减少流程摩擦）

### D2：Worker-Worker 1:1 通信中，Admin 可见是什么程度？

- **可见但不拦截**：Admin 能在 UI 看到所有消息记录，但不实时干预
- **实时告警**：特定关键词（如"删除"、"rm"）触发实时提醒
- **当前倾向**：可见但不拦截 + 危险操作词实时告警

---

*本文档记录截止 2026-03-13。架构确认后请在此处标记"实施中"并更新版本号。*
